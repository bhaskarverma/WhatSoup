...
def setup_selenium():
    ...
    DRIVER_PATH = '/home/apollyon/Desktop/Projects/Fufaji/Whatsapp Extractor/Git/chromedriver'
    CHROME_PROFILE = '/home/apollyon/Library/Application Support/Google/Chrome/Default'
...
def scrape_chat(driver):
    ...
    message_list = driver.find_element_by_xpath(
        '//*[@id="main"]/div[3]/div/div[2]/div[2]').get_attribute('class')  
...
def parse_datetime(text, time_only=False):
    ...
    if not time_only:
        for fmt in ("%d/%m/%Y %H:%M %p", '%Y-%m-%d %H:%M %p'):
            try:
